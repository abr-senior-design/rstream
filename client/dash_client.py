...
def get_mpd(url):
    ...
    connection.close()
    mpd_file = "temp/" + url.split('/')[-1]
    make_sure_path_exists(os.path.dirname(mpd_file))
...
def id_generator(id_size=6):
    """ Module to create a random string with uppercase
        and digits.
    """
    return 'temp/' + ''.join(random.choice(ascii_letters+digits) for _ in range(id_size))


def download_segment(segment_url, dash_folder, seg_size):
    """ Module to download the segment """
    try:
        request="{}@{}".format(segment_url, seg_size)
...
#----------------------------------------------------------------------------------------
        ...
        start_time = timeit.default_timer()
        try:
            segment_size, segment_filename = download_segment(segment_url, file_identifier, dp_object.video[current_bitrate].segment_sizes[segment_number])
...
DEFAULT_URL="http://127.0.0.1:8000/sample_mpd/BigBuckBunny_4s.mpd"
